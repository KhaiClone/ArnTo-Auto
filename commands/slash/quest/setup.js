...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                { name: "⏱ Thời gian chờ QR", value: "10 phút (quá thời gian sẽ hết hạn).", inline: true },
                { name: "🏦 Thanh toán", value: "Chuyển đúng mã nội dung `Quest...` để hệ thống tự xác nhận.", inline: true }
            )
            .setTimestamp();
        ...
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId("quest:enter_token").setLabel("Nhập token").setStyle(ButtonStyle.Primary)
        );

        await interaction.channel.send({ embeds: [embed], components: [row] });
        await interaction.editReply({ content: "✅ Đã gửi panel vào kênh này.", ephemeral: true });
    },
};
