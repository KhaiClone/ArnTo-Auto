    ...
    async execute(interaction, client) {
        ...
        if (interaction.isButton() && interaction.customId.startsWith("panel:")) {
            ...
            if (!bots || bots.length === 0) {
                return interaction.editReply({ content: "You do not have any bots." });
            }
        ...
        if (interaction.isButton() && interaction.customId.startsWith("panel_action:")) {
            const [, action, botId] = interaction.customId.split(":");
            await interaction.deferReply({ ephemeral: true });

            try {
                const result = await client.autoPanel.performAction(botId, action);
        ...
        if (interaction.isModalSubmit() && interaction.customId.startsWith("panel_modal:")) {
            ...
            if (actionType === "extend") {
                const monthsStr = interaction.fields.getTextInputValue("months");
                value = parseInt(monthsStr, 10);
                if (isNaN(value) || value <= 0) {
                    return interaction.editReply({ content: "Invalid number of months." });
                }
                ...
                const ramStr = interaction.fields.getTextInputValue("additionalRam");
                value = parseInt(ramStr, 10);
                if (isNaN(value) || value <= 0 || value % 64 !== 0) {
                    return interaction.editReply({ content: "Invalid RAM amount. Must be divisible by 64 (e.g. 64, 128)." });
                }

                let remainingMonths = 1;
                if (bot.expiresAt) {
                    const msRemaining = Math.max(0, bot.expiresAt - Date.now());
                    remainingMonths = Math.ceil(msRemaining / (30 * 24 * 60 * 60 * 1000));
            ...
            client.autoBank.createQR(amount, transferCode, pendingData.context, async (err, data) => {
                if (err) {
                ...
                try {
                    if (data.context.type === "extend") {
                        await client.autoPanel.extendBot(data.context.botId, data.context.value);
                        client.autoPanel._notifyUser(data.context.userId, `✅ Payment received! Your bot has been extended by **${data.context.value}** months.`);
                    } else if (data.context.type === "upgrade") {
                        await client.autoPanel.upgradeBot(data.context.botId, data.context.value);
                        client.autoPanel._notifyUser(data.context.userId, `✅ Payment received! Your bot's RAM has been upgraded by **${data.context.value}** MB.`);
                    }
                } catch (e) {
                    client.autoPanel._notifyUser(data.context.userId, `❌ Payment received, but an error occurred while applying the upgrade. Please contact support. (Bot ID: ${data.context.botId})`);
                }
            });
            ...
            const embed = new EmbedBuilder()
                .setTitle("Thanh toán " + (actionType === "extend" ? "Gia hạn" : "Nâng cấp"))
                .setDescription([
                    description,
                    "",
